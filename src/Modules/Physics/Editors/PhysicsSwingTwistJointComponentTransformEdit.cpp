#include "PhysicsSwingTwistJointComponentTransformEdit.h"
...
TransformComponent *
PhysicsSwingTwistJointComponentTransformEdit::getTransformComponentByName(const std::string &name) const {
    if (name.empty()) {
...
PhysicsSwingTwistJointComponentTransformEdit::TransformOption
PhysicsSwingTwistJointComponentTransformEdit::getTransformTargetOptions(int index) {
    switch (index) {
        case 1:
            return {
                "Attachment point A",
...
glm::mat4 PhysicsSwingTwistJointComponentTransformEdit::getWorldSpaceTransform(int index) {
...
void PhysicsSwingTwistJointComponentTransformEdit::setWorldSpaceTransform(glm::mat4 m, int index) const {
    if (m_linkedTransformA == nullptr || m_linkedTransformB == nullptr) {
        return;
    }
